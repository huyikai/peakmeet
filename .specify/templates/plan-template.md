...
PeakMeet Constitution v1.3.0 — 全部勾选方可继续；NON-NEGOTIABLE 项不可豁免。
...
- [ ] **Compliance**: 无医疗表述；含免责声明要求；隐私最小必要；无极端节食/超负荷推荐；第三方媒体仅使用宪章已记录的明确例外
- [ ] **Source & Media Provenance**: 外部内容固定来源与哈希；catalog 单向同步 CloudBase；原始数据、转换/enrichment、许可状态分层且可审计
- [ ] **Brand & Visual**: 面向用户 UI 对齐 `docs/brand`；计算页遵循视觉契约/共享样式；未另起主色板；未引入重型 UI 组件库
