...
落地 PeakMeet Constitution 约定的 TypeScript 优先 pnpm monorepo 工程底座：根工具链（tsconfig / ESLint / Prettier / Vitest）与统一脚本；初始化 `shared`、`miniprogram`、`web`、`cloudfunctions` 骨架及 `database/`、`docs/`；shared 双产物（ESM `dist/` + CJS `dist-cjs/`）与 `.d.ts`；小程序四 Tab + `build:miniprogram`（sync + tsc→js）供开发者工具预览；官网四页 SSG 并 workspace 引用 shared 冒烟；云函数含 1 个可编译占位函数；`docs/constitution.md` 完整落盘；真实 AppID 仅存本地 private 配置。
...
**Constraints**: TypeScript 优先；禁止 Taro/uni-app/重型框架/自建后端/第三方 BaaS/Astro SSR；纯计算不进云函数；小程序不纳入根 `build`（使用 `build:miniprogram`）；shared 零第三方运行时依赖；无持续 watch 同步；真实微信 AppID 不进 Git 跟踪文件
...
### Post-implement doc sync (2026-07-23)

- [x] 已回写：shared ESM+CJS、`build:miniprogram`、开发者工具需 `.js`、AppID 仅 private 配置（见 research / contracts / quickstart）
...
├── packages/
│   ├── shared/
│   │   ├── src/                 # 纯 TS（含冒烟导出 getPeakMeetPing）
│   │   ├── __tests__/           # Vitest *.test.ts
│   │   ├── dist/                # ESM + .d.ts（Web）
│   │   ├── dist-cjs/            # CJS（sync → 小程序）
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   └── tsconfig.cjs.json
│   ├── miniprogram/
│   │   ├── pages/{home,train,diet,mine}/  # *.ts 源码 + tsc 产出 *.js
│   │   ├── utils/shared/        # sync 目标（CJS，gitignore 内容）
│   │   ├── app.ts / app.js / app.json / app.wxss
│   │   ├── project.config.json  # touristappid + typescript compiler plugin
│   │   ├── project.private.config.json  # 本地 AppID（gitignore）
│   │   ├── tsconfig.json
...
├── scripts/
│   └── sync-shared-to-miniprogram.mjs  # 拷贝 dist-cjs → utils/shared
├── tsconfig.base.json
├── eslint.config.js
├── prettier.config.js
├── vitest.workspace.ts
├── pnpm-workspace.yaml
├── package.json                 # test, lint, format, build, sync:shared, build:miniprogram, dev:web
└── README.md
```

**Structure Decision**: 本功能触及全部约定顶层目录。`scripts/` 为根级工程脚本（非业务顶层），用于 shared→miniprogram 同步。小程序预览依赖 `build:miniprogram` 产出 `.js`，不新增 Constitution 未列的业务顶层目录。
