...
- Q: 内容如何更新（开源仓库）？ → A: **推荐方案（006 当时）**：`database/seeds` 为唯一内容源；通过**本地/CI 同步脚本**导入开发者自己的云环境；**禁止**普通小程序客户端调用全量刷库同步；不为公开仓库配套可被任意调用的「写公共库」接口；不当 CMS
- **008 修订**：公共内容权威源改为仓库 `database/catalog/`（由 `database/vendor/` 固定快照生成）；动作规模与媒体改为 exercises-dataset 固定提交；正式迁移可替换残留文档。详见 `specs/008-exercises-dataset-migration/`。
...
- **FR-007**: 公共内容的权威源 MUST 为仓库 `database/catalog/`（由固定 vendor 快照生成；006 时代的 `database/seeds` 已退役为历史输入）；更新流程 MUST 为改 catalog → 版本管理 → **本地或 CI 同步**到目标云环境
